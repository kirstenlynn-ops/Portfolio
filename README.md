# Hi! My Name is Kirsten Livingston!🙌
I'm a emerging data scientist passionate about turning raw data into actionable insights. 

## Skills and Proficiencies
**Data Analysis:📚**

-Python, R, SQL

-Data Cleaning & Wrangling

-Exploratory Data Analysis (EDA)

-Predictive Modeling & Machine Learning

**Data Visualization:📈**

-Matplotlib, Tableau, Seaborn, Plotly

-Interactive Dashboards

**Tools and Plateforms:🔨**

-GitHub

-Jupyter Notebook, RStudio

-Excel, Google Sheets
| Programming & Analysis | Data Visualization | Data Science Skills |
|------------------------|-----------------|------------------|
| Python                 | Tableau         | Data Cleaning    |
| R                      | Matplotlib      | EDA              |
| SQL                    | Seaborn         | Predictive Modeling |
## Skills and Proficiencies
| Data Analysis:📚                     | Data Visualization:📈       | Tools and Plateforms:🔨   |
|---------------------------------------|-----------------------------|----------------------------|
| Python, R, SQL                        | Matplotlib                  | Github                     |
| Data Cleaning & Wrangling             | Tableau                     | Jupyter Notebook           |           
| Exploratory Data Analysis             | Seaborn                     | RStudio                    |
| Predictive Modeling & Machine Learning| Interactive Dashboards      | Excel & Google Sheets      |

**Data Visualization:📈**

-Matplotlib, Tableau, Seaborn, Plotly

-Interactive Dashboards

**Tools and Plateforms:🔨**

-GitHub

-Jupyter Notebook, RStudio

-Excel, Google Sheets
